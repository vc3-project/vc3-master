...
class HandleHeadNodes(VC3Task):
    ...
    def check_timeout(self, request, next_state, reason):
        ...
        if diff > self.node_max_no_contact_time:
            ...
            reason = reason + " (Headnode could not be contacted. This may be a transient error. Waiting for {} seconds before declaring failure.)".format(self.node_max_no_contact_time - diff)
            return (next_state, reason)
        else:
            return (next_state, reason)
